...
public class UpdateUserUseCaseTest
{
    ...
    {
        var ex = await Assert.ThrowsAsync<UserNotFoundException>(() => _useCase.Execute(JungleContextMock.User1Request,11));
    ...
    {
        await  _useCase.Execute(JungleContextMock.User1Request,1);
        _userRepository.Verify(x => x.UpdateUser(new UserDto(JungleContextMock.User1Request, 1)), Times.Once);
        _unitOfWork.Verify(x => x.Save(), Times.Once);
    }

    private void ConfigureUserRepositoryForExistingLogin()
    {
        this._userRepository.Setup(repo => repo.UpdateUser(new UserDto(JungleContextMock.User1Request, 1))).
            ReturnsAsync(true);

        this._userRepository.Setup(repo => repo.UpdateUser(new UserDto(JungleContextMock.User1Request, 11))).
            ReturnsAsync(false);
    }
